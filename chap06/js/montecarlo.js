...
var guisetup = () => {
  ...
  document.querySelector("#b05").addEventListener( "click", () => {
    nl.emit( "stop", null );
    nl.emit( "max", { "top":1000000 } );
  });

  document.querySelector("#d02").addEventListener( "click", () => {
    nl.emit( "stop", null );
    nl.emit( "dimension", { "dimension":2, "answer":Math.PI } );
  });
  document.querySelector("#d03").addEventListener( "click", () => {
    nl.emit( "stop", null );
    nl.emit( "dimension", { "dimension":3, "answer":4.0/3.0*Math.PI } );
  });
  document.querySelector("#d04").addEventListener( "click", () => {
    nl.emit( "stop", null );
    nl.emit( "dimension", { "dimension":4, "answer":Math.PI*Math.PI/2.0 } );
  });
  document.querySelector("#d05").addEventListener( "click", () => {
    nl.emit( "stop", null );
